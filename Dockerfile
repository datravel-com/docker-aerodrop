...
RUN echo 'deb https://deb.nodesource.com/node_0.10 wheezy main' > /etc/apt/sources.list.d/nodesource.list


RUN apt-get update \
 && apt-get install -y mercurial bison make curl wget binutils gcc python-software-properties  build-essential git acl --no-install-recommends \
 && apt-get clean \
 && rm -rf /var/lib/apt/lists/* /tmp/* /var/tmp/*
RUN mkdir server
